...
def report():
    ...
    report_query="select s.f_name,s.l_name,sub.name,a.attendance,a.date from students s natural join attend a natural join subjects sub where s.std_id=a.std_id and a.sub_id=sub.sub_id and sub.name=%s and date between %s and %s" ;
    ...
    line=['First Name,Last Name,Subject Name,Attendance,Date']
    writer.writerow(line)

    for row in result:
        #line=[str(row['s.f_name'])+','+str(row['s.l_name'])+','+str(row['sub.name'])+','+str(row['a.attendance'])+','+str(row['a.date'])]
